...
def get_transform_csc(pts_hw, o_size):
    pts_h, pts_w = pts_hw
    h, w = o_size[0], o_size[1]
    t = np.zeros((3,3))
    t[0,0] = float(w) / pts_w
    t[1,1] = float(h) / pts_h
    t[2,2] = 1
...
def transform_pixel_csc(pt, pts_hw, output_size, invert=0):
    t = get_transform_csc(pts_hw, output_size)
    if invert:
        t = np.linalg.inv(t)
...
def transform_preds_csc(coords, input_size, output_size):
    for p in range(coords.size(0)):
        coords[p, 0:2] = torch.tensor(transform_pixel_csc(coords[p, 0:2], input_size, output_size, 1))
    return coords
